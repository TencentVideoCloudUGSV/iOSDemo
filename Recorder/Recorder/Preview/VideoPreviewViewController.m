...
-(void) onPlayEvent:(TXVodPlayer *)player event:(int)EvtID withParam:(NSDictionary*)param
{
    ...
    UISlider *slider = _sdPreviewSlider;
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (EvtID == PLAY_EVT_PLAY_PROGRESS) {
            ...
            if (duration > 0 && slider.maximumValue != duration) {
            ...
            self->_progressTipLabel.text = progressTips;
            return ;
        } else if(EvtID == PLAY_EVT_PLAY_END) {
            [slider setValue:0];
            [self->_voidPlayer resume];
            
            [self->_btnStartPreview setImage:[UIImage imageNamed:@"pausepreview"] forState:UIControlStateNormal];
            [self->_btnStartPreview setImage:[UIImage imageNamed:@"pausepreview_press"] forState:UIControlStateSelected];
            self->_progressTipLabel.text = @"00:00/00:00";
        }
    });
}
